...
constexpr bool static_equal(const concepts::shape auto& left, const concepts::shape auto& right)
{
...
constexpr bool equal(const concepts::shape auto& left, const concepts::shape auto& right)
{
	using left_type = decltype(left);
	using right_type = decltype(right);
