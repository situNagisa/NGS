...
private:
	...
	void _RecordAllocate(T* block, __size count) {
		...
		nos << TextColor::WHITE << "申请"
			<< TextColor::YELLOW << info.size << " bytes"
			<< TextColor::WHITE << "内存，\t地址： "
			<< (block ? TextColor::WHITE : TextColor::RED) << (void*)block
			<< TextColor::WHITE << "\t类型："
			<< TextColor::CYAN << info.std_info->name()
			<< TextColor::RESERT;
		;
		if (count != 1)
			nos.Trace("[%d]", count);
	...
	void _RecordFree(void* block, __size count = 1) {
		...
		nos << TextColor::WHITE << ("释放")
			<< TextColor::GREEN << info.size << " bytes"
			<< TextColor::WHITE << "内存，\t地址： "
			<< (block ? TextColor::WHITE : TextColor::RED) << (void*)block
			<< TextColor::WHITE << "\t类型："
			<< TextColor::CYAN << info.std_info->name()
...
//接口
template<_CPT Different<void*> T>
void
MemorySwap(T* a, T* b) {
