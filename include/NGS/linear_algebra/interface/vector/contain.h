...
namespace detail
{
	...
	{
		if constexpr (requires(_T target, traits::vectors::index_t index) { {interfaces::vectors::contains(NGS_PP_PERFECT_FORWARD(target), index)} -> std::convertible_to<bool>; })
		{
			return functor_choice::interface;
		}
		else if constexpr (requires(_T target, traits::vectors::index_t index) { {target.contains(index)} -> std::convertible_to<bool>; })
		{
...
{
	constexpr decltype(auto) operator()(callable_contain auto&& target, traits::vectors::index_t index) const
	{
		constexpr auto choice = detail::contains_choice<decltype(target)>();
		if constexpr (choice == functor_choice::interface)
		{
			return interfaces::vectors::contains(NGS_PP_PERFECT_FORWARD(target), index);
		}
