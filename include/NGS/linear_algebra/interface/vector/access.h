...
namespace detail
{
	...
	{
		if constexpr (requires(_T target, traits::vectors::index_t index) { interfaces::vectors::access(NGS_PP_PERFECT_FORWARD(target), index); })
		{
			return functor_choice::interface;
		}
		else if constexpr (requires(_T target, traits::vectors::index_t index) { target.access(index); })
		{
			return functor_choice::member;
		}
		else if constexpr (requires(_T target, traits::vectors::index_t index) { target[index]; })
		{
...
{
	constexpr decltype(auto) operator()(accessible auto&& target, traits::vectors::index_t index) const
	{
...
using value_t = type_traits::object_t<decltype(adapters::vectors::access(declval<_Type>(), declval<index_t>()))>;
NGS_MATH_LA_VECTOR_TRAIT_END