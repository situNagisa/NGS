...
NGS_mfunction(transform, class _Rng, template<class...>class _Transformer, class = std::make_index_sequence<_Rng::size>);
/**
 * \brief Transform a vector into another vector using a transformer.
 * 
 * \tparam _Transformer transform(range[i]) -> result_type
 */
NGS_mfunction(_vector_transform_1, CVector _Rng, template<class>class _Transformer, size_t... _Index) {
 ...
 */
NGS_mfunction(_vector_transform_2, CVector _Rng, template<class,class>class _Transformer, size_t... _Index) {
	using result_type = replace_template_parameters_t<_Rng, typename _Transformer<_Rng,ccpt::uint_<_Index>>::result_type...>;
};

NGS_mfunction(transform, CVector _Rng, template<class...>class _Transformer, size_t... _Index) < _Rng, _Transformer, std::index_sequence<_Index...> > {
