...
public:
	...
	esp_uart(self_type&&) = default;
	~esp_uart()
	{
		if (base_type::base().valid())
		{
			auto result = ::uart_driver_delete(base_type::base().value());
			NGS_OSAPI_ESP_CHECK_ERROR_THROW_MESSAGE(result, "Failed to delete uart driver");
		}
	}
