...
{
	return static_cast<::std::underlying_type_t<privilege>>(left) <=> static_cast<::std::underlying_type_t<privilege>>(right);
}
...
{
	auto privilege = NGS_LIB_NAME::high(pde_privilege, pte_privilege);
	auto access = NGS_LIB_NAME::low(pde_access, pte_access);
