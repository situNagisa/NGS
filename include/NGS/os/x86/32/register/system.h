...
public:
	...
	using register_category_type = registers::gdtr;

	::std::uint16_t size;
	::std::uint16_t address_low;
	::std::uint16_t address_high;

	constexpr pointer_underlying_t address() const noexcept
	{
...
public:
	...
	using register_category_type = registers::ldtr;
...
public:
	using underlying_type = ::std::uint64_t;
	using register_category_type = registers::idtr;
