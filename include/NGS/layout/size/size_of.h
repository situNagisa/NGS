...
constexpr size_t size_of(const std::array<size_t, _Count>& sizes) {
	auto offsets = offset<_Align>(sizes);
...
constexpr size_t size_of(std::integral auto... sizes) { return size_of<_Align>(std::array{ static_cast<size_t>(sizes)... }); }
...
constexpr size_t size_of() { return size_of(size_of<_Types>()...); }

template<size_t _Align,class... _Types>
	requires (sizeof...(_Types) > 1)
constexpr size_t size_of() { return size_of<_Align>(size_of<_Types>()...); }
