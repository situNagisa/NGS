...
template <class _Type, _Type _Value, template<class...>class _Bitmap, class... _Items>
struct _bitmap_find<_Type, _Value, _Bitmap<_Items...>> {
private:
	using _result = find_if<typename _bitmap_find_predicate<_Type, _Value>::meta_function, _Items ... >;
public:
...
private:
	...
	using _transfomated = template_apply_transform_t<std::tuple<_Args...>, boost::mpl::map, _item_transformer>;
public:
	template<class _Key>
	constexpr static auto value = boost::mpl::at<type, _Key>::type::value;
	template<_Type _Value>
	using key = typename _bitmap_find<_Type, _Value, template_apply_transform_t<std::tuple<_Args...>, boost::mpl::map, _item_transformer>>::type;
};
