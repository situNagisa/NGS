...
public:
	...
	NGS_TYPE_DEFINE(Point2<Number>, vector);
...
public:
	...
	__this_ref operator|=(__this_ref_cst rect) {
		...
		x = std::min(Left(), rect.Left());
		width = std::max(right, rect.Right()) - Left();
		y = std::min(Top(), rect.Top());
		height = std::max(bottom, rect.Bottom()) - Top();
		return *this;
	}
	__this_ref operator&=(__this_ref_cst rect) {
		if (!Intersect(rect)) {
		...
		__num right = Right(), bottom = Bottom();
		x = std::max(Left(), rect.Left());
		width = std::min(right, rect.Right()) - Left();
		y = std::max(Top(), rect.Top());
