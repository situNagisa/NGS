...
struct assign_function_impl
{
	constexpr auto operator()(CScalarContainer auto& left, CScalarExpression auto&& right) const
		requires requires{ { left.assign(NGS_PERFECT_FORWARD(right)) }; }
	{
		...
		return left.assign(
			static_cast<type_traits::set_same_cvref_t<right_type, left_type>>(
				static_cast<right_type>(NGS_PERFECT_FORWARD(right))
				)
		);
	}

	constexpr auto operator()(CScalarContainer auto& left,auto&& right) const
		requires type_traits::is_same_object_v<meta::scalar_value_t<decltype(left)>,decltype(right)> && requires{ { left.assign(NGS_PERFECT_FORWARD(right)) }; }
	{
		return left.assign(NGS_PERFECT_FORWARD(right));
	}
};
