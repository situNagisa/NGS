...
constexpr auto CHOICE()
{
	if constexpr (requires
	{
		...
		requires _Concept<functor::vector_value_t<_Left>, functor::vector_value_t<_Right>>::value;
	})
	{
	...
	{
		requires functor::expression<_Left>;
		requires _Concept<functor::vector_value_t<_Left>, _Right>::value;
	})
	{
	...
	{
		requires functor::expression<_Right>;
		requires _Concept<_Left, functor::vector_value_t<_Right>>::value;
	})
	{
...
struct arithmetic_functor
{
	...
	{
		return functor::binary_functor<decltype(left),decltype(right),_Access>{NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right)};
	}
	constexpr decltype(auto) operator()(auto&& left, auto&& right) const
...
{
	return functor::access(NGS_PERFECT_FORWARD(left), index) + functor::access(NGS_PERFECT_FORWARD(right), index);
} > {};
...
constexpr decltype(auto) operator+(auto&& left, auto&& right)
	requires requires{ detail_arithmetic::add(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right)); }
{
	return  detail_arithmetic::add(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right));
}

constexpr decltype(auto) operator-(auto&& left, auto&& right)
	requires requires { detail_arithmetic::subtract(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right)); }
{
	return  detail_arithmetic::subtract(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right));
}

constexpr decltype(auto) operator*(auto&& left, auto&& right)
	requires requires { detail_arithmetic::multiply(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right)); }
{
	return  detail_arithmetic::multiply(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right));
}

constexpr decltype(auto) operator/(auto&& left, auto&& right)
	requires requires { divide(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right)); }
{
	return  detail_arithmetic::divide(NGS_PERFECT_FORWARD(left), NGS_PERFECT_FORWARD(right));
}
