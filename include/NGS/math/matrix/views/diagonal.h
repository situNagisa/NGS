...
namespace _detail
{
	inline constexpr struct
	{
		...
		{
			return (major == minor) * vectors::random_access(NGS_PP_PERFECT_FORWARD(vector), major);
		}
	}basic_diagonal_matrix_transformer{};

	template<extent_t _Extent, class _T>
		requires (_Extent != adapter_extent)
	using basic_diagonal_matrix = transform_view<_Extent, _Extent, basic_diagonal_matrix_transformer,
		vectors::default_sizer<_Extent>(), vectors::default_sizer<_Extent>(), vectors::vector<_T, _Extent>>;
