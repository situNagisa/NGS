...
using row_dimension_t = size_t;
using col_dimension_t = size_t;
NGS_MATH_LA_MATRIX_TRAIT_END
NGS_MATH_LA_MATRIX_ADAPTER_BEGIN
inline constexpr traits::matrixes::dimension_t dimension_dynamic = static_cast<traits::vectors::dimension_t>(-1);
inline constexpr traits::matrixes::dimension_t dimension_invalid = static_cast<traits::vectors::dimension_t>(-2);
...
namespace detail
{
	...
	constexpr functor_choice dimension_choice()
	{
		...
		{
			if constexpr (requires(_T target) { {dimension(NGS_PP_PERFECT_FORWARD(target))} -> std::convertible_to<traits::vectors::dimension_t>; })
			{
...
template<class> constexpr traits::vectors::dimension_t static_dimension = dimension_invalid;
template<valid_dimension _T>
constexpr traits::vectors::dimension_t static_dimension<_T> = type_traits::object_t<_T>::dimension;
