...
public:
	...
	explicit(sizeof...(Args) == 0) buffer_group(Args&&... buffer_constructor_args)
		: self_type(::std::make_tuple(Buffer{ NGS_PP_PERFECT_FORWARD(buffer_constructor_args)... }...))
	{}
	...
	struct wrapper_type
	{
		...
		constexpr const wrapper_type& operator=(const value_type& vertex) const
		{
	...
	void describe(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage)
	{
		stl::tuples::for_each(_group, [usage](auto&& buffer)
			{
				NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::bind(NGS_PP_PERFECT_FORWARD(buffer).get_context());
...
template<cpt::derived_from_specialization<buffer>... Buffer>
buffer_group(const ::std::tuple<Buffer...>&) -> buffer_group<Buffer...>;

template<cpt::derived_from_specialization<buffer>... Buffer>
buffer_group(::std::tuple<Buffer...>&&) -> buffer_group<Buffer...>;
...
template<cpt::derived_from_specialization<vertex_buffer>... Buffer>
vertex_group(const ::std::tuple<Buffer...>&) -> vertex_group<Buffer...>;

template<cpt::derived_from_specialization<vertex_buffer>... Buffer>
vertex_group(::std::tuple<Buffer...>&&) -> vertex_group<Buffer...>;

template<::std::size_t VertexCount, buffer_descriptor... Buffers>
decltype(auto) make_array_vertex_group(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage)
{
	return vertex_group<vertex_buffer<::std::array<Buffers, VertexCount>>...>{usage};
}

template<::std::size_t VertexCount, buffer_descriptor... Buffers>
decltype(auto) make_span_vertex_group(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage, ::std::span<Buffers,VertexCount>... buffers)
{
	return vertex_group{::std::make_tuple(NGS_LIB_MODULE_NAME::make_span_vertex_buffer(usage, buffers)...)};
}

template<buffer_descriptor... Buffers>
decltype(auto) make_span_vertex_group(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage, ::std::span<Buffers>... buffers)
{
	return vertex_group{ ::std::make_tuple(NGS_LIB_MODULE_NAME::make_span_vertex_buffer(usage, buffers)...) };
}

template<::std::size_t VertexCount, buffer_descriptor... Buffers>
decltype(auto) make_span_vertex_group(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage, type_traits::add_cv_like_t<Buffers,void>*... buffers)
{
	return vertex_group{::std::make_tuple(NGS_LIB_MODULE_NAME::make_span_vertex_buffer<Buffers,VertexCount>(usage, buffers)...) };
}

template<buffer_descriptor... Buffers>
decltype(auto) make_span_vertex_group(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage, ::std::size_t vertex_count, type_traits::add_cv_like_t<Buffers,void>*... buffers)
{
	return vertex_group{::std::make_tuple(NGS_LIB_MODULE_NAME::make_span_vertex_buffer<Buffers>(usage, buffers, vertex_count)...) };
}

template<buffer_descriptor... Buffers>
decltype(auto) make_vector_vertex_group(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage)
{
	return vertex_group<vertex_buffer<::std::vector<Buffers>>...>{usage};
}

template<buffer_descriptor... Buffers>
decltype(auto) make_vector_vertex_group(NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::usage usage, ::std::size_t vertex_count)
{
	return vertex_group<vertex_buffer<::std::vector<Buffers>>...>{usage, vertex_count};
}
