...
namespace _detail
{
	...
	{
		size_t count;
		NGS_EXTERNAL_OPENGL_BASIC_ENUM_NS::buffer_vertex_fundamental type;
		bool normalized = false;
		size_t stride = 0;
		size_t offset = 0;
	};

	constexpr auto make_attribute(const NGS_EXTERNAL_OPENGL_BASIC_REFLECT_NS::buffer_vertex_data& data, bool normalized, size_t stride)
	{
	...
	struct make_attribute_from_variables<Buffer, ::std::index_sequence<Index...>>
	{
		static constexpr auto make(bool normalized)
		{
			...
			auto field_data = mpl::mstruct::reflect<Buffer, NGS_EXTERNAL_OPENGL_BASIC_REFLECT_NS::buffer_vertex_reflecter>();
	...
	auto install_buffer(cpt::naked_same_as<NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::vertex_buffer> auto&& buffer, size_t index, ::std::span<const attribute> attributes)
	{
		using descriptor_type = Descriptor;
		NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::bind(NGS_PP_PERFECT_FORWARD(buffer));
	...
	template<buffer_descriptor... Descriptor>
	void pipeline_layout(const NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::vertex_array& vertex_array, bool normalized, cpt::naked_same_as<NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::vertex_buffer> auto&&... buffers)
	...
	{
		static void call(const NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::vertex_array& vertex_array, bool normalized, cpt::naked_same_as<NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::vertex_buffer> auto&&... buffers)
		{
			_detail::pipeline_layout<mpl::mstruct::variable_value_t<type_traits::naked_t<Ts>>...>(vertex_array, normalized, NGS_PP_PERFECT_FORWARD(buffers)...);
		}
	};

	void pipeline_layout(const NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::vertex_array& vertex_array, bool normalized, cpt::derived_from_specialization<vertex_group> auto&& group)
	{
		::std::apply([&](auto&&... buffers)
			{
				call_pipeline_layout<mpl::mstruct::fields_t<typename type_traits::object_t<decltype(group)>::vertex_type>>::call(vertex_array, normalized, NGS_PP_PERFECT_FORWARD(buffers).get_context()...);
...
struct vertex_array
{
	using context_type = NGS_EXTERNAL_OPENGL_BASIC_CONTEXT_NS::vertex_array;
